...
// reads a 4B int (uint32_t) raw binary buffer and advances the pointer
...
//reads the fixed len str from buffer into OUT and advances the pointer
...
// In-memory key-value store (lives only while server runs; created by SET, retrieved by GET, erased by DEL)
static void do_request(vector<string> &cmd ,Response &out){
	if(cmd.size()==2 && cmd[0]=="get"){
		...
		out.data.assign(val.begin(),val.end());
			...
			out.status = RES_ERR; // unrecognized cmd
	}
}
...
static void handle_read(Conn *conn){
	...
	if(rv<0){
		msg_errno("read() error"); // handle IO error
		conn->want_close=true;
	}
	// handle EOF
	if(rv==0){
